...
def create_clustered_bubble_map(data, indicator_col, geographic_level):
    """Create clustered bubble map for better visualization of dense areas"""
    ...
    if group_col in data.columns and indicator_col in data.columns:
        ...
        if len(agg_data) > 0:
            ...
            fig = px.scatter_mapbox(
                ...
                title=f'📍 {indicator_col.replace("_", " ").title()} - {geographic_level} Level',
...
def create_comparative_analysis_map(hotspot_data):
    ...
    if hotspot_data is not None and len(hotspot_data) > 0:
        ...
        fig = make_subplots(
            ...
            subplot_titles=(
                ...
                '⚖️ Underweight Rate', '🎯 Composite Risk'
            ),
        ...
        fig.add_trace(
            go.Scattermapbox(
                lat=hotspot_data['lat_first'],
                lon=hotspot_data['lon_first'],
                mode='markers',
                marker=dict(
                    size=hotspot_data['stunting_rate_mean'] * 50 + 10,
                    color=hotspot_data['stunting_rate_mean'],
                    colorscale='Reds',
                    showscale=True,
                    colorbar=dict(x=0.45, y=0.8, len=0.15)
                ),
                text=hotspot_data['district'] + '<br>Stunting: ' + (hotspot_data['stunting_rate_mean'] * 100).round(1).astype(str) + '%',
                hoverinfo='text',
                name='Stunting'
            ),
            row=1, col=1
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                mode='markers',
                marker=dict(
                    size=hotspot_data['wasting_rate_mean'] * 80 + 10,
                    color=hotspot_data['wasting_rate_mean'],
                    colorscale='Oranges',
                    showscale=True,
                    colorbar=dict(x=0.95, y=0.8, len=0.15)
                ),
                text=hotspot_data['district'] + '<br>Wasting: ' + (hotspot_data['wasting_rate_mean'] * 100).round(1).astype(str) + '%',
                hoverinfo='text',
                name='Wasting'
            ),
            row=1, col=2
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                mode='markers',
                marker=dict(
                    size=hotspot_data['underweight_rate_mean'] * 60 + 10,
                    color=hotspot_data['underweight_rate_mean'],
                    colorscale='Purples',
                    showscale=True,
                    colorbar=dict(x=0.45, y=0.2, len=0.15)
                ),
                text=hotspot_data['district'] + '<br>Underweight: ' + (hotspot_data['underweight_rate_mean'] * 100).round(1).astype(str) + '%',
                hoverinfo='text',
                name='Underweight'
            ),
            row=2, col=1
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                mode='markers',
                marker=dict(
                    size=hotspot_data['composite_risk_score'] * 60 + 10,
                    color=hotspot_data['composite_risk_score'],
                    colorscale='RdYlBu_r',
                    showscale=True,
                    colorbar=dict(x=0.95, y=0.2, len=0.15)
                ),
                text=hotspot_data['district'] + '<br>Risk Score: ' + hotspot_data['composite_risk_score'].round(3).astype(str),
                hoverinfo='text',
                name='Composite Risk'
            ),
            row=2, col=2
        ...
        fig.update_layout(
            mapbox1=dict(
                ...
                center=dict(lat=-1.9441, lon=30.0619),
                zoom=7
            ),
            mapbox2=dict(
                style="open-street-map",
            ...
            height=800,
            title_text="🔍 Comparative Analysis: Multiple Nutrition Indicators",
            title_x=0.5,
            title_font_size=20,
            showlegend=False
        )
...
def main():
    ...
    with st.sidebar:
        ...
        viz_type = st.selectbox(
            "🎨 Visualization Type",
            ["Severity Classification", "Interactive Bubble Map", "Advanced Heat Map", 
    ...
    with col1:
        ...
        if viz_type == "Severity Classification":
            ...
            fig = create_comparative_analysis_map(child_hotspots)
        else:
            fig = create_enhanced_bubble_map(child_geo, indicator, geographic_level)
